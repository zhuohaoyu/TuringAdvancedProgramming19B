...
///// needed in the SupportInfo /////
Function* SDDG::getFunc(){
    return mFunc ;
}
 
DenseMap<Instruction *, SDDGNode *>& SDDG::getItstNodes(){
    return mInterestingNodes ;
}

set<pair<Instruction *, Instruction *>>& SDDG::getShares(){
    return mShares ;
}
...
void dotifyToFile(DenseMap<Instruction *, SDDGNode *> &nodes, set<pair<Instruction *, Instruction *>> &shares, string &file, bool showShareRelations, bool hashed = false) {
    ...
    for (auto iter = nodes.begin(); iter != nodes.end(); iter++) {
        ...
        if (hashed) {
            fos << normalizeInstruction(inst) << "\"];" << endl;
        } else {
            string label;
